...
class Noise:
      ...
      def overlay_images(
            ...
            """
            # 背景画像の読み込み
            background = cv2.imread(path_functions_instance.absolutePath(background_path))
            if background is None:
                  raise FileNotFoundError(f"Background image not found: {background_path}")
            background = cv2.cvtColor(background, cv2.COLOR_BGR2RGB)

            # オーバーレイ画像の読み込み
            overlay = cv2.imread(path_functions_instance.absolutePath(overlay_path))
            overlay = cv2.cvtColor(overlay, cv2.COLOR_BGR2RGB)
            if overlay is None:
                  raise FileNotFoundError(f"Overlay image not found: {overlay_path}")

            # 画像を重ねる
            result = cv2.addWeighted(background, 1, overlay, alpha, 0)
            ...
            cv2.imwrite(path_functions_instance.absolutePath(output_path), result)
